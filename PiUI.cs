...
public class PiUI : MonoBehaviour
{
    ...
    /// </summary>
    public enum TransitionType { SlideLeft, SlideRight, SlideUp, SlideDown, Scale, Fan, ScaleAndFan };
    ...
    public void SetSliceType(int i,UIType type, UnityEvent anEvent, Sprite pic, int size)
    {
        piData[i].SetUIType(type);
        piData[i].SetEvent(anEvent);
        piData[i].SetImg(pic);
        piData[i].SetImgSize(size);
    }

    public void SetSliceLabel(string s,UIType type)
    {
        if (piData == null || piData.Length <= 0)
        {
            return;
        }
        PiData p = Array.Find(piData, _p => _p.GetUIType() == type);
        if (p != null)
        {
            p.SetLabel(s);
        }
    ...
    public void OpenMenu(Vector2 screenPos)
    {
        ...
        switch (openTransition)
        {
            ...
            case TransitionType.SlideRight:
                ...
                break;
            case TransitionType.SlideLeft:
                tempPos.x = Screen.width + outerRadius;
                transform.localScale = Vector2.one * scaleModifier;
                transform.position = tempPos;
                break;
            case TransitionType.SlideUp:
                tempPos.y = -outerRadius;
                transform.localScale = Vector2.one * scaleModifier;
                transform.position = tempPos;
                break;
            case TransitionType.SlideDown:
                tempPos.y = Screen.height + outerRadius;
                transform.localScale = Vector2.one * scaleModifier;
                transform.position = tempPos;
                break;
            case TransitionType.Fan:
    ...
    public class PiData
    {
        ...
        public void SetEvent(UnityEvent anEvent)
        {
            this.onSlicePressed = anEvent;
        }
        ...
        public void OnInspectorGUI(SerializedProperty sprop, PiUI menu, System.Action AddSlice, System.Action<PiData> RemoveSlice, System.Action<int> angleUpdate)
        {
            ...
            if (icon == null)
                icon = Sprite.Create(angleTexture,new Rect(0,0,angleTextureSize,angleTextureSize),Vector2.zero,1f,0,SpriteMeshType.FullRect,Vector4.zero,false);
